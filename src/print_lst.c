...
int				print_lst(t_list *list, int limit, t_specif *now_basket, int *i)
{
	int				in;
	int				count;
	unsigned char	*tmp;
	...
	*i = *i + now_basket->skip;
	...
	while (list && (!limit ? 1 : in < limit))
	{
		...
		ft_strncpy(tmp, (unsigned char*)(list->content), list->content_size);
...
unsigned char	*ft_strncpy(unsigned char *dst, unsigned char *src, size_t len)
{
	size_t	i;
